...
parser.add_argument("input_file", nargs="?", default="model.puml",
...
for src, src_card, rel_type, dst_card, dst, label in relation_pattern.findall(content):
    ...
    dst_card = dst_card if dst_card else "0..*"
    ...
    with open(java_file, "w") as out:
        ...
        if class_name in relations_dict:
            for target, card, rel_type, label, role in relations_dict[class_name]:
                ...
                comment = ""
                if rel_type == "-->":
                    comment = f"association directionnelle avec {target}"
                elif rel_type == "--":
                    comment = f"association bidirectionnelle avec {target}"
                elif rel_type == "o--":
                    comment = f"agrégation avec {target}"
                elif rel_type == "*--":
                    comment = f"composition avec {target}"
                else:
                    comment = f"relation avec {target}"
                if label:
                    comment += f" ('{label}')"
